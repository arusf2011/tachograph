<?php
    ...
    if($count_convoys > 0)
    {
        ...
        {
            $f3->set('c_start_name',$db->exec('SELECT name FROM cities WHERE id = '.$convoys[$i]['c_start']));
            $c_start_name = $f3->get('c_start_name');
    
            $f3->set('comp_start_name',$db->exec('SELECT name FROM companies WHERE id = '.$convoys[$i]['comp_start']));
            $comp_start_name = $f3->get('comp_start_name');
    
            $f3->set('c_end_name',$db->exec('SELECT name FROM cities WHERE id = '.$convoys[$i]['c_end']));
            $c_end_name = $f3->get('c_end_name');
    
            $f3->set('comp_end_name',$db->exec('SELECT name FROM companies WHERE id = '.$convoys[$i]['comp_end']));
                ...
                <div class="col-xl-4 col-md-6 mb-4">
                    <div class="card border-left-primary shadow h-100">
                        ...
                        <div class="card-body">
                            <div class="row no-gutters align-items-center">
                                <div class="col mr-2">
                                    ...
                                    <p class="small text-primary mb-1"><?php echo $f3->get('time_groupup').' - '.date($global_settings[9]['value'],strtotime($convoys[$i]['groupup'])) ?></p>
                        ...
                        <div class="card-footer form-inline">
                            <a href="./edit_convoy/<?= $convoys[$i]['id'] ?>"><button class="btn btn-warning"><i class="fas fa-edit"></i> <?= $f3->get('edit') ?></button></a>
                            <button href="javascript:void(0);" data-href="./delete_convoy/<?= $convoys[$i]['id'] ?>" class="btn btn-danger lookup_btn"><i class="fas fa-trash"></i></button>
                            <button href="javascript:void(0);" data-href="./convoy/<?= $convoys[$i]['id'] ?>" class="btn btn-secondary lookup_btn"><i class="fas fa-info-circle"></i></button>
                        </div>
                    </div>
                </div>
            <?php
