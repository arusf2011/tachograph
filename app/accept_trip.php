<?php
    ...
    $type = 'accepted_trip_'.$trip_id;
    $f3->set('admin_id',$db->exec('SELECT id FROM users WHERE nickname = ?',$_SESSION['nickname']));
    $admin_id = $f3->get('admin_id');
    $f3->set('change_status',$db->exec('UPDATE trips SET status = 3, id_admin = ? WHERE id = ?',array($admin_id[0]['id'],$trip_id)));
