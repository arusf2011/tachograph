<?php
  $f3=Base::instance();
  session_start();
  $f3->set('id_user',$db->exec('SELECT id FROM users WHERE nickname = ?',$_SESSION['nickname']));
  $id_user = $f3->get('id_user');
  $f3->set('trips_arr',$db->exec('SELECT * FROM trips WHERE id_user = '.$id_user[0]['id']));
  $trips = $f3->get('trips_arr');
...
<head>
  ...
  <title><?php echo $f3->get('dashboard_trips_title'); ?></title>
      ...
      <li class="nav-item">
        ...
        <div id="trips" class="collapse show" aria-labelledby="headingPages" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
            ...
            <a class="collapse-item active text-<?= $global_settings[17]['value'] ?>" href="./trips_user"><i class="fas fa-clipboard-list fa-fw"></i> <?= $f3->get('list') ?></a>
          </div>
        </div>
      </li>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          <?php
            if(isset($_SESSION['success']) && $_SESSION['success'] == 'add_trip')
            {
              ?>
                <div class="alert alert-success">
                  <?= $f3->get('addtrip_success') ?>
                </div>
              <?php
                    ...
                    <?php
                      foreach($trips as $trip)
                      {
                        ...
                        ?>
                          <tr>
                            <td><?= $trip['id'] ?></td>
                            <td><?= $city_start[0]['name'].' - '.$comp_from[0]['name'] ?></td>
                            <td><?= $city_end[0]['name'].' - '.$comp_to[0]['name'] ?></td>
                            <td>
                              <?php
                                $date_beg = date_create($trip['date_beg']);
                                $date_end = date_create($trip['date_end']);
                                $diff_date = date_diff($date_beg,$date_end);
                                echo $diff_date->format("%H h %i min");
                              ?>
                            </td>
                            <td><span style="text-transform: capitalize;"><?php echo $f3->get($trip['load_type']).'</span> - '.$trip['tonnage'].' '.$global_settings[6]['value']; ?></td>
                            <td>
                              <?php
                                switch($trip['status'])
                                {
                            ...
                            <td class="text-center"><button href="javascript:void(0);" class="btn btn-primary lookup_btn" data-href="./trip_user/<?= $trip['id'] ?>"><i class="fas fa-map"></i> <?= $f3->get('lookup') ?></button></td>
                          </tr>
                        <?php
                      }
                    ?>
        ...
        <!-- End of Main Content -->
        <div id="image_end_div" class="modal_img_end">

        <!-- The Close Button -->
        <span class="close_img_end">&times;</span>

        <!-- Modal Content (The Image) -->
        <img class="modal-content_img_end" id="img_modal">
  ...
  <script>
    $(document).ready(function(){
      $('.lookup_btn').on('click',function(){
          ...
          $('.modal-body-trip').load(data_href,function(){
