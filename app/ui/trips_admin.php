<?php
  $f3=Base::instance();
  session_start();
  $f3->set('trips_arr',$db->exec('SELECT * FROM trips'));
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          <?php
            if(isset($_SESSION['success']) && $_SESSION['success'] == 'accepted_trip')
            {
              ?>
                <div class="alert alert-success">
                  <?= $f3->get('accepted_trip') ?>
                </div>
              <?php
              unset($_SESSION['success']);
            }
            else if(isset($_SESSION['success']) && $_SESSION['success'] == 'rejected_trip')
            {
              ?>
                <div class="alert alert-success">
                  <?= $f3->get('rejected_trip') ?>
                </div>
              <?php
              unset($_SESSION['success']);
            }
                  ...
                  <tbody>
                    <?php
                      foreach($trips as $trip)
                      {
                        ...
                        $f3->set('trip_user_nickname',$db->exec('SELECT nickname FROM users WHERE id = "'.$trip['id_user'].'"'));
                        ...
                        ?>
                          <tr>
                            ...
                            <td><?= $f3->get($trip['load_type']).' - '.$trip['tonnage'].' '.$global_settings[6]['value'] ?></td>
                            <td><?= $trip_user_nickname[0]['nickname'] ?></td>
                            <td class="text-center"><button href="javascript:void(0);" class="btn btn-primary lookup_btn" data-href="./trip_admin/<?= $trip['id'] ?>" <?php if($trip_user_nickname[0]['nickname'] == $_SESSION['nickname']) echo 'disabled' ?> ><i class="fas fa-map"></i> <?= $f3->get('lookup') ?></button></td>
                          </tr>
                        <?php
                      }
                    ?>
