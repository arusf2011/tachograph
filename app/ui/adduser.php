...
<head>
  ...
  <title><?php echo $f3->get('dashboard_adduser_title'); ?></title>
      ...
      <li class="nav-item">
        ...
        <div id="collapseTwo" class="collapse show" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
            ...
            <a class="collapse-item active text-<?= $global_settings[17]['value'] ?>" href="./add_user"><i class="fas fa-user-plus fa-fw"></i> <?php echo $f3->get('add'); ?></a>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          <h1 class="page-title"><?php echo $f3->get('adduser') ?></h1>
          <?php 
            if(isset($_SESSION['error']) && $_SESSION['error'] == '1')
            {
              ?>
              <div class="alert alert-danger"><?= $f3->get('rules_not_accepted') ?></div>
              <?php
            }
          ...
          ?>
          <form method="POST" action="./new_user_add">
            <div class="row">
                <div class="col">
                  <label for="nickname" class="mt-2"><?php echo $f3->get('nickname') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input type="text" class="form-control" name="nickname" required>
                  <label for="email" class="mt-2"><?php echo $f3->get('email') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <small for="email" class="mt-2"><?php echo $f3->get('email_clarification') ?></small>
                  <input type="email" class="form-control" name="email" required>
                  <label for="dlcs" class="mt-2"><?php echo $f3->get('dlcs_owned') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <select name="dlcs" class="form-control" required multiple size="6">
                    <?php
                  ...
                  <label for="rules" class="mt-3"><?php echo $f3->get('rules_accepted') ?><span style="color:red;font-size: 20px;">*</span></label>
                ...
                <div class="col">
                  <label for="ats"><?php echo $f3->get('ats_owner') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <select name="ats" class="form-control" required>
                    <option value=""><?php echo $f3->get('select_option') ?></option>
                    <option value="0"><?php echo $f3->get('no') ?></option>
                    <option value="1"><?php echo $f3->get('yes') ?></option>
                  </select>
                  <label for="truck_ets2"><?php echo $f3->get('truck_ets2') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <select name="truck_ets2" class="form-control" required>
                    <option value=""><?php echo $f3->get('select_option') ?></option>
                    <?php
                      $f3->set('trucks_ets2',$db->exec('SELECT * FROM trucks WHERE game = false'));
                      $trucks_ets2 = $f3->get('trucks_ets2');
                      foreach($trucks_ets2 as $truck_ets2)
                      {
                        ?>
                        <option value="<?= $truck_ets2['short_name'] ?>"><?= $truck_ets2['name'] ?></option>
                        <?php
                      }
                    ?>
                  </select>
                  <label for="truck_ats"><?php echo $f3->get('truck_ats') ?></label>
                  <select name="truck_ats" class="form-control">
                    <option value=""><?php echo $f3->get('select_option') ?></option>
                    <?php
                      $f3->set('trucks_ats',$db->exec('SELECT * FROM trucks WHERE game = true'));
                      $trucks_ats = $f3->get('trucks_ats');
                      foreach($trucks_ats as $truck_ats)
                      {
                        ?>
                        <option value="<?= $truck_ats['short_name'] ?>"><?= $truck_ats['name'] ?></option>
                        <?php
                      }
                    ?>
                  </select>
                  <p><?php echo $f3->get('adduser_disclaimer') ?></p>
                  <label for="tmp_id">TruckersMP ID</label>
                  <input type="number" class="form-control" name="tmp_id">
                </div>
            </div>
            <div class="text-center mt-2">
              <button class="btn btn-success"><i class="fas fa-user-plus"></i> <?php echo $f3->get('adduser') ?></button>
            </div>
          </form>
        </div>
      </div>
