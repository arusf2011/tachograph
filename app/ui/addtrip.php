<?php
  ...
  $f3->set('truck_ets2',$db->exec('SELECT name FROM trucks WHERE short_name = ?',$user_data[0]['truck_ets2']));
  $truck_ets2 = $f3->get('truck_ets2');
  $f3->set('truck_ats',$db->exec('SELECT name FROM trucks WHERE short_name = ?',$user_data[0]['truck_ats']));
  $truck_ats = $f3->get('truck_ats');
?>
<!DOCTYPE html>
<html lang="en">
...
<body id="page-top">
  ...
  <div id="wrapper">
    ...
    ?>
      ...
      <!-- Nav Item - Pages Collapse Menu -->
      <li class="nav-item">
        <a class="nav-link" href="#" data-toggle="collapse" data-target="#trips" aria-expanded="true" aria-controls="collapsePages">
          <i class="fas fa-fw fa-address-book"></i>
          <span><?= $f3->get('trips') ?></span>
        </a>
        <div id="trips" class="collapse show" aria-labelledby="headingPages" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded">
            <a class="collapse-item active text-<?= $global_settings[17]['value'] ?>" href="./add_trip"><i class="fas fa-plus-square fa-fw"></i> <?= $f3->get('add_trip') ?></a>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid bg-light">
          <h1 class="page-title"><?php echo $f3->get('add_trip') ?></h1>
          <?php 
          ...
          ?>
          <form method="POST" action="./new_trip_add" enctype="multipart/form-data">
            <div class="text-center checkbox my-2">
                <select id="game" class="form-control">
                  <option value=""><?= $f3->get('select_game') ?></option>
            ...
            <div class="row">
                <div class="col-sm-6">
                  <label for="from_city"><?php echo $f3->get('from') ?><span style="color:red;font-size: 20px;">*</span></label>
                  ...
                  <label for="load" class="mt-2"><?php echo $f3->get('load') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <select class="form-control" name="load" id="load" required data-live-search="true">
                  </select>
                  <label for="tonnage" class="mt-2"><?php echo $f3->get('tonnage') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input type="number" class="form-control" name="tonnage" id="tonnage" required>
                  <label for="money" class="mt-2"><?php echo $f3->get('money') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input type="number" class="form-control" name="money" id="money" required>
                </div>
                <div class="col-sm-6">
                  <label for="used_truck"><?php echo $f3->get('used_truck') ?></label>
                  <input type="text" disabled class="form-control bg-white border-secondary" id="show_used_truck">
                  <input type="hidden" id="truck_ets2" value="<?= $truck_ets2[0]['name'] ?>">
                  <input type="hidden" id="truck_ets2_sn" value="<?= $user_data[0]['truck_ets2'] ?>">
                  <input type="hidden" name="used_truck" id="used_truck" value="">
                  <input type="hidden" id="truck_ats_sn" value="<?= $user_data[0]['truck_ats'] ?>">
                  <input type="hidden" id="truck_ats" value="<?= $truck_ats[0]['name'] ?>">
                  <label for="distance" class="mt-2"><?php echo $f3->get('distance') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input type="number" class="form-control" name="distance" id="distance" required>
                  <label for="fuel_used" class="mt-2"><?php echo $f3->get('fuel_used_trip') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input type="text" class="form-control" name="fuel_used" id="fuel_used" required>
                  <label for="datepicker1" class="mt-3"><?= $f3->get('datetime_beg') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input id="datepicker1" />
                  <input type="hidden" name="datetime_beg" id="datetime_beg">
                  <label for="datepicker2" class="mt-3"><?= $f3->get('datetime_end') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input id="datepicker2" />
                  <input type="hidden" name="datetime_end" id="datetime_end">
                  <label for="damage" class="mt-2"><?php echo $f3->get('damage') ?><span style="color:red;font-size: 20px;">*</span></label>
                  <input type="number" class="form-control" name="damage" id="damage" required>
                </div>
            </div>
            <div class="container text-center mt-1">
              <label for="image_end"><?= $f3->get('upload_image_end') ?><span style="color:red;font-size: 20px;">*</span></label><br>
              <input type="file" id="image_end" name="{image_end}" accept="image/png,image/jpeg"><br>
              <label for="participate_convoy"><?= $f3->get('participated_convoy') ?></label>
              <select name="participate_convoy" id="participate_convoy" class="form-control">
                <option value="0"><?= $f3->get('none') ?></option>
                <?php
                  $f3->set('convoys',$db->exec('SELECT id, name FROM convoys WHERE date_beg_convoy BETWEEN date(now() - INTERVAL 2 DAY) and date(now())'));
                  $convoys = $f3->get('convoys');
                  $count_convoys = count($convoys);
                  for($i = 0;$i < $count_convoys; $i++)
                  {
                    ?>
                      <option value="<?= $convoys[$i]['id'] ?>"><?= $convoys[$i]['name'] ?></option>
                    <?php
                  }
                ?>
              </select>
            ...
            <div class="text-center mt-1">
              <button class="btn btn-success"><i class="fas fa-plus-square"></i> <?php echo $f3->get('add_trip') ?></button>
            </div>
          </form>
        </div>
      </div>
  ...
  <script>
    $("#game").change(function() {
        ...
        xmlhttp_load.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
            document.getElementById("load").innerHTML = this.responseText;
            document.getElementById("load").classList.add('selectpicker');
            $('#load').selectpicker({
              style: 'btn-default border-secondary'
            });
          }
        };
        xmlhttp_load.open("GET", "./load_loads/"+game, true);
        xmlhttp_load.send();
        ...
        if(game == '0')
        {
          document.getElementById('show_used_truck').value = document.getElementById('truck_ets2').value;
          document.getElementById('used_truck').value = document.getElementById('truck_ets2_sn').value;
        }
        else
        {
          document.getElementById('show_used_truck').value = document.getElementById('truck_ats').value;
          document.getElementById('used_truck').value = document.getElementById('truck_ats_sn').value;
        }
    });
    $('#datepicker1').datetimepicker({
      ...
      change: function () {
        var datepicker1 = $('#datepicker1').datetimepicker();
        document.getElementById('datetime_beg').value = datepicker1.value();
      }
    });
    ...
    $('#datepicker2').datetimepicker({
      ...
      format: 'yyyy-mm-dd HH:MM',
      change: function () {
        var datepicker2 = $('#datepicker2').datetimepicker();
        document.getElementById('datetime_end').value = datepicker2.value();
      }
    });
